...
def get_pretrained_embedding(speaker, folder):
    return [np.load(os.path.join(hp.preprocessed_path, folder, filename)) for filename in os.listdir(os.path.join(hp.preprocessed_path, folder)) if speaker in filename]
