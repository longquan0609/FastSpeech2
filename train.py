...
def main(args):
    ...
    scheduled_optim = ScheduledOptim(optimizer, hp.decoder_hidden, hp.n_warm_up_step, hp.aneal_steps, hp.aneal_rate, args.restore_step, )
            ...
            try:
                for j, data_of_batch in enumerate(batchs):
                    ...
                    if current_step % hp.log_step == 0:
                        ...
                        str2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Mel PostNet Loss: {:.4f}, Duration Loss: {:.4f}, F0 Loss: {:.4f}, Energy Loss: {:.4f};".format(t_l, m_l, m_p_l, d_l, f_l, e_l)
                        str3 = ("Time Used: {:.3f}s, Estimated Time Remaining: {:.3f}s.".format((Now - Start), (total_step - current_step) * np.mean(Time)))
                    ...
                    if current_step % hp.save_step == 0:
                        torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), }, os.path.join(checkpoint_path, "checkpoint_{}.pth.tar".format(current_step), ), )
                    ...
                    if current_step % hp.synth_step == 0:
                        ...
                        mel_postnet = (mel_postnet_output[0:1, :mel_length].detach().transpose(1, 2))

                        utils.vocoder_infer(mel, vocoder, [os.path.join(hp.synth_path, "step_{}_wo_postnet_{}.wav".format(current_step, basename), )], )
                        utils.vocoder_infer(mel_postnet, vocoder, [os.path.join(hp.synth_path, "step_{}_with_postnet_{}.wav".format(current_step, basename), )], )
                        utils.vocoder_infer(mel_target, vocoder, [os.path.join(hp.synth_path, "step_{}_reconstruct_{}.wav".format(current_step, basename), )], )
